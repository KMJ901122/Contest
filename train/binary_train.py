...
def model_train(dataset, datagen=True, mixup=False):

    normal_path=parent_dir+'/datasets/normal'
    abnormal_path=parent_dir+'/datasets/'+dataset
    ...
    checkpoint_path=parent_dir+'/weights/'+dataset+'/{epoch:03d}.ckpt'
    checkpoint_dir=os.path.dirname(checkpoint_path)
    cp_callback=keras.callbacks.ModelCheckpoint(filepath=checkpoint_path, save_weights_only=True, verbose=1, save_freq=1)
    ...
    if datagen:
        ...
        model.fit(train_data, val_data, batch_size=8, epochs=1, callbacks=[reduce_lr, cp_callback])
