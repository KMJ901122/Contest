...
# Callback
checkpoint_path=parent_dir+'/weights/breast/{epoch:03d}.ckpt'
...
def create_model(opt):
    input_tensor=keras.layers.Input((config.IMG_SHAPE, config.IMG_SHAPE, 4))
...
normal_image_path.sort()
abnormal_image_path.sort()
...
normal_train=int(normal_total*config.SPLIT)//4
abnormal_train=int(abnormal_total*config.SPLIT)//4
...
train_data=mg_train_gen(normal_image_path, abnormal_image_path, normal_train, abnormal_train)
val_data=mg_val_gen(normal_image_path, abnormal_image_path, normal_train, abnormal_train)
total_data=mg_total_gen(normal_image_path, abnormal_image_path)

train_ds=tf.data.Dataset.from_generator(train_data, (tf.float32, tf.float32), ((config.IMG_SHAPE, config.IMG_SHAPE, 4), (2))).cache().batch(1).prefetch(AUTO)
val_ds=tf.data.Dataset.from_generator(val_data, (tf.float32, tf.float32), ((config.IMG_SHAPE, config.IMG_SHAPE, 4), (2))).cache().batch(1).prefetch(AUTO)

total_ds=tf.data.Dataset.from_generator(total_data, (tf.float32, tf.float32), ((config.IMG_SHAPE, config.IMG_SHAPE, 4), (2))).cache().batch(1).prefetch(AUTO)

datagen=tf.keras.preprocessing.image.ImageDataGenerator(rotation_range=6, height_shift_range=0.12,
                                                       vertical_flip=True, zoom_range=0.12, shear_range=0.12,)
...
if config.TRAIN_WITH_MIXUP:
    train_with_mixup(model, train_data, val_ds, batch_size, epochs, callbacks=[reduce_lr, cp_callback])
